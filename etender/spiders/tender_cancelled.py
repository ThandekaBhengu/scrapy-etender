...
class TendersSpider(scrapy.Spider):
    ...
    start_urls = ['https://etenders.treasury.gov.za/content/cancelled-tenders']
    ...
    def parse(self, response):
        ...
        for tender_row in tender_table.xpath('tr'):
            ...
            yield {
                ...
                'tender_notice': tender_row.css('td.views-field-title::text').get().strip(),
                'number': tender_row.css('td.views-field-field-code::text').get().strip(),
                'reason_cancelled': tender_row.css('td.views-field-field-reason-cancellation p::text').get()
    ...
    def parse_tender(self, response):
        ...
        etender['tender_notice'] = response.meta['tender_notice']
        etender['number'] = response.meta['number']
        etender['reason_cancelled'] = response.meta['reason_cancelled']
